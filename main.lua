...
-- [[ 🗝️ KEY SYSTEM & MAIN WINDOW / ウィンドウ作成 ]]
...
KeyTab:AddParagraph({
    Title = "Access Verification / 認証",
    Content = "Please get the key from the link below.\n下のボタンからキーを取得してください。"
})
...
KeyTab:AddButton({
    ...
    Callback = function()
        setclipboard(KeyURL)
        Fluent:Notify({ Title = "System", Content = "Link Copied! / コピーしました", Duration = 5 })
    end
})
...
KeyTab:AddButton({
    ...
    Callback = function()
        if KeyInput == LatestKey and not Loaded then
            Loaded = true
            Fluent:Notify({ Title = "Success", Content = "Unlocking Features... / ロード中...", Duration = 3 })
            StartMainFeatures()
        elseif Loaded then
            Fluent:Notify({ Title = "Info", Content = "Already Loaded / 既にロード済みです", Duration = 3 })
        else
...
function StartMainFeatures()
    ...
    local WalkSpeed, bS, Target = 16, 5, ""
    ...
    rs.Heartbeat:Connect(function()
        ...
        for _, v in pairs(game.Players:GetPlayers()) do
            if v ~= lp and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if dist < 20 and BAToggle.Value then v.Character.HumanoidRootPart.CFrame = char.HumanoidRootPart.CFrame * CFrame.new(0,0,-3) end
    ...
    uis.InputBegan:Connect(function(input, gpe)
        if not gpe and input.KeyCode == Enum.KeyCode.P and HamToggle.Value then
            ...
            task.spawn(function() for i=1,10 do b.Size = b.Size + Vector3.new(5,5,5); b.Transparency = i/10; task.wait() end b:Destroy() end)
                    ...
                    if (root.Position - v.Character.HumanoidRootPart.Position).Magnitude < 20 then
                        ...
                        for _, p in pairs(v.Character:GetChildren()) do 
                            if p:IsA("BasePart") then 
                                ...
                                task.delay(1, function() pcall(function() p.Color = oldColor p.Material = "Plastic" end) end) 
                            end 
                        end
                        -- デカ赤文字 BANNED!!!
            ...
            Fluent:Notify({ Title = "BAN", Content = "ADMIN POWER ACTIVATED!!!", Duration = 2 })
        end
    end)
...
Window:SelectTab(1) -- 最初はKeyタブを表示
